...
## Outputs topics:
* *The topic names have been renamed from the original names for clarity in the purpose of a given topic.*
* *Topics can be found using **rostopic list** then using **rostopic echo \<topic\>**.*
* **/IMUdata** (sensor\_msgs/Imu) -- fused IMU data
* **/IMUraw** (sensor\_msgs/Imu) -- raw accelerometer data
* **/IMUmag** (sensor\_msgs/MagneticField) -- raw magnetic field data
* **/IMUtemp** (sensor\_msgs/Temperature) -- temperature data
* **/IMUstatus** (diagnostic\_msgs/DiagnosticStatus) -- a DiagnosticStatus object showing the current calibration, interrupt, system status of the IMU
