...
class MotorInterface(Node):
    ...
    def __init__(self) -> None:
        ...
        self.subscription = self.create_subscription(
            Float32MultiArray, "motor_powers", self.callback, qos
        )
    ...
    def logging_function(self):
        while not self.stop_event.is_set():
            self.logger().info(
                f"Motor Goals: {self.motor_commander.logical_pin_targets}"
            )
            self.logger().info(
                f"Motor States: {self.motor_commander.logical_pin_states}"
            )
