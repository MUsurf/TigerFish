...
pca.frequency = 350
ACTUAL_PCA_FREQUENCY = 370.2 # Measure this with an oscilloscope
...
class MotorCommander:
    ...
    def set_motor_power(
        self, motor_power: float, motor_index: int, deadband: float = 0.0
    ) -> int:
        if motor_index >= NUM_MOTORS or motor_index < 0:
            return None
        motor_power = max(-1.0, min(1.0, motor_power))

        if abs(motor_power) <= deadband:
            pulse_us = MED_WIDTH
        else:
            pulse_us = (
                (MED_WIDTH + motor_power * (MED_WIDTH - MIN_WIDTH))
                if motor_power < 0
                else (MED_WIDTH + motor_power * (MAX_WIDTH - MED_WIDTH))
            )
