...
class StateEstimator(Node):
    def __init__(self):
        ...
        self.imu_subscriber = self.create_subscription(
            Imu, "processed_imu_data", self._imu_cb, 10
        )
    ...
    def _timer_cb(self):
        if self.current_odometry is not None:
            self.odometry_publisher.publish(self.current_odometry)
    ...
    def _imu_cb(self, msg: Imu):
        ...
        o.twist.twist.linear.x = (
            msg.linear_acceleration.x * dt + self.current_odometry.twist.twist.linear.x
        )
        o.twist.twist.linear.y = (
            msg.linear_acceleration.y * dt + self.current_odometry.twist.twist.linear.y
        )
        o.twist.twist.linear.z = (
            msg.linear_acceleration.z * dt + self.current_odometry.twist.twist.linear.z
        )
        ...
        o.pose.pose.position.x = (
            ...
            + 0.5
            * (self.current_odometry.twist.twist.linear.x + o.twist.twist.linear.x)
            * dt
        )
        o.pose.pose.position.y = (
            self.current_odometry.pose.pose.position.y
            + 0.5
            * (self.current_odometry.twist.twist.linear.y + o.twist.twist.linear.y)
            * dt
        )
        o.pose.pose.position.z = (
            self.current_odometry.pose.pose.position.z
            + 0.5
            * (self.current_odometry.twist.twist.linear.z + o.twist.twist.linear.z)
            * dt
        )
