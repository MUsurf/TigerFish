...
class PIDNode(Node):
    def __init__(self,
             ...
             pitch_gains=(0.011, 0.02, 0.03),
             yaw_gains=(0.01, 0.0001, 0.02)):
        ...
        self.pid_input_subscriber = self.create_subscription(
            PIDInput, "pid_input", self.input_cb, 10
        )
        ...
        self.motor_publisher = self.create_publisher(
            Float32MultiArray, "motor_powers", motor_qos
        )

        self.orientation_subscriber = self.create_subscription(
            Odometry, "state_estimation", self.odom_cb, 10
        )
    ...
    def timer_cb(self):
        ...
        x_pow = (
            self.last_msg.x_power
            if not self.last_msg.x_mode
            else self.x_pid(self.last_msg.x_setpoint - self.last_msg.x_measurement, dt)
        )

        y_pow = (
            self.last_msg.y_power
            if not self.last_msg.y_mode
        ...
        motor_powers[4:8] *= (
            0.707  # adjust the z to reflect how we lose some of our power on the x and y axis
        )
    ...
    def new_controller_cb(self, msg: Float32MultiArray):
        ...
        msg = msg.data
        if msg[0] < 0.5:
            self.x_pid = PIDController(msg[1], msg[2], msg[3])
        elif msg[0] < 1.5:
            self.y_pid = PIDController(msg[1], msg[2], msg[3])
        elif msg[0] < 2.5:
            self.z_pid = PIDController(msg[1], msg[2], msg[3])
        elif msg[0] < 3.5:
            self.roll_pid = PIDController(msg[1], msg[2], msg[3])
        elif msg[0] < 4.5:
            self.pitch_pid = PIDController(msg[1], msg[2], msg[3])
        elif msg[0] < 5.5:
            self.yaw_pid = PIDController(msg[1], msg[2], msg[3])
