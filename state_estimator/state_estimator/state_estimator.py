...
class StateEstimator(Node):
    def __init__(self):
        ...
        self.sub_data = self.create_subscription(Imu, "imu_data", self.on_data, 10)

        self.sub_raw = self.create_subscription(Imu, "imu_raw", self.on_raw, 10)

        self.last_time = None

        # state
        self.q_body = np.array([1.0, 0.0, 0.0, 0.0])  # orientation
