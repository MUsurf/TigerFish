...
class PIDController:
    ...
    def calculate(self, error: float, dt: float) -> float:
        power = self.kP * error
        power += (
            (error - self.last_error) / dt * self.kD
            if self.last_error is not None
            else 0.0
        )
        power += self.accumulated_error * self.kI
        self.accumulated_error += error * dt
        if self.integral_max is not None and abs(self.accumulated_error) > abs(
            self.integral_max
        ):
            if self.accumulated_error < 0:
                self.accumulated_error = -abs(self.integral_max)
            else:
                self.accumulated_error = abs(self.integral_max)
...
class PIDNode(Node):
    def __init__(
        ...
        self.position_setpoint_subscriber = self.create_subscription(
            Setpoint, "position_setpoints", self.position_setpoint_cb, qos
        )
        self.velocity_setpoint_subscriber = self.create_subscription(
            Setpoint, "velocity_setpoints", self.velocity_setpoint_cb, qos
        )
        self.position_measurement_subscriber = self.create_subscription(
            Setpoint, "position_measurements", self.position_setpoint_cb, qos
        )
        self.velocity_measurement_subscriber = self.create_subscription(
            Setpoint, "velocity_measurements", self.velocity_setpoint_cb, qos
        )
        self.pid_modes = self.create_subscription(Mode, "pid_modes", self.mode_cb, qos)
        self.motor_publisher = self.create_publisher(
            Float32MultiArray, "motor_powers", qos
        )
        ...
        self.pitch_pos_controller = PIDController(
            pitch_kP_pos, pitch_kI_pos, pitch_kD_pos
        )
        self.pitch_vel_controller = PIDController(
            pitch_kP_vel, pitch_kI_vel, pitch_kD_vel
        )
    ...
    def timer_callback(self):
        ...
        x_setpoint = (
            self.x_pos_controller(
                self.position_setpoints.x - self.position_measurements.x, dt
            )
            if self.modes.x
            else self.velocity_setpoints.x
        )
        y_setpoint = (
            self.y_pos_controller(
                self.position_setpoints.y - self.position_measurements.y, dt
            )
            if self.modes.y
            else self.velocity_setpoints.y
        )
        z_setpoint = (
            self.z_pos_controller(
                self.position_setpoints.z - self.position_measurements.z, dt
            )
            if self.modes.z
            else self.velocity_setpoints.z
        )
        roll_setpoint = (
            self.roll_pos_controller(
                self.position_setpoints.roll - self.position_measurements.roll, dt
            )
            if self.modes.roll
            else self.velocity_setpoints.roll
        )
        pitch_setpoint = (
            self.pitch_pos_controller(
                self.position_setpoints.pitch - self.position_measurements.pitch, dt
            )
            if self.modes.pitch
            else self.velocity_setpoints.pitch
        )
        yaw_setpoint = (
            self.yaw_pos_controller(
                self.position_setpoints.yaw - self.position_measurements.yaw, dt
            )
            if self.modes.yaw
            else self.velocity_setpoints.yaw
        )

        x_pow = self.x_to_motor(
            self.x_vel_controller(x_setpoint - self.velocity_measurements.x, dt)
        )
        y_pow = self.y_to_motor(
            self.y_vel_controller(y_setpoint - self.velocity_measurements.y, dt)
        )
        z_pow = self.z_to_motor(
            self.z_vel_controller(z_setpoint - self.velocity_measurements.z, dt)
        )
        roll_pow = self.roll_to_motor(
            self.roll_vel_controller(
                roll_setpoint - self.velocity_measurements.roll, dt
            )
        )
        pitch_pow = self.pitch_to_motor(
            self.pitch_vel_controller(
                pitch_setpoint - self.velocity_measurements.pitch, dt
            )
        )
        yaw_pow = self.yaw_to_motor(
            self.yaw_vel_controller(yaw_setpoint - self.velocity_measurements.yaw, dt)
        )
