...
from pid_node.msg import Measurement, Mode
...
class Main(Node):
    def __init__(self):
        ...
        self.imu_subscriber = self.create_subscription(
            Float32MultiArray, "processed_imu_data", self.imu.update, qos
        )

        self.mode_publisher = self.create_publisher(Mode, "pid_modes", 10)
        self.pos_measurement_publisher = self.create_publisher(
            Measurement, "position_measurements", 10
        )
        self.vel_measurement_publisher = self.create_publisher(
            Measurement, "velocity_measurements", 10
        )
