...
class ServoControllerNode(Node):
	...
	def __init__(self):
		...
		self.subscription = self.create_subscription(Float64, self.subscribedTopic, self.angle_callbackFunction, self.queueSize)
		self.subscription # Prevent unused variable warning
  
		self.publisherTopic='topic_servo_angle_feedback'
		# Create the publisher, which publishes a new desired position
		self.feedback_publisher = self.create_publisher(Float64, publisherTopic, queueSize)
		
	# Convert angle in degrees to PWM duty cycle
	def angle_to_duty_cycle(self, angle):
		angle = max(self.min_angle, min(angle, self.max_angle)) # Ensure angle is within our defined range
